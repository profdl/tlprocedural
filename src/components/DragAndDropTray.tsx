...
import {
  bezierShapeToCustomTrayItem,
  generateBezierThumbnailSvg,
} from "./utils/bezierToCustomShape";
import {
  combineShapesToCustom,
  type MultiShapeChildDefinition,
  cloneShapeProps,
  sanitizeMeta,
  RESERVED_META_KEYS,
...
const lucideIcons = {
  ...
  triangle:
    '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13.73 4a2 2 0 0 0-3.46 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/></svg>',
  pentagon:
    '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3.5 8.7c-.7.5-1 1.4-.7 2.2l2.8 8.7c.3.8 1 1.4 1.9 1.4h9c.9 0 1.6-.6 1.9-1.4l2.8-8.7c.3-.8 0-1.7-.7-2.2L12 2.2c-.7-.5-1.7-.5-2.4 0Z"/></svg>',
  circle:
    '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/></svg>',
...
function buildChildShapesFromDefinition(
  ...
  return childDefinitions.map((child) => {
    ...
    const metaExtras =
      options.customShapeId && options.instanceId
        ? {
...
export function DragAndDropTray() {
  ...
  const isCustomShape = useCallback(
    (itemId: string) => {
  ...
  const handleCreateCustomShape = useCallback(() => {
    ...
    if (
      ...
      validSelectedShapes[0].type === "bezier"
    ) {
    ...
    if (!customTrayItem.iconSvg || customTrayItem.iconSvg.trim().length === 0) {
      if (customTrayItem.shapeType === "bezier") {
        const bezierDefaults = customTrayItem.defaultProps as
          | Partial<BezierShape["props"]>
          | undefined;
        const points = bezierDefaults?.points;
        if (Array.isArray(points) && points.length > 0) {
          customTrayItem = {
            ...customTrayItem,
            iconSvg: generateBezierThumbnailSvg(
              points,
              !!bezierDefaults?.isClosed
            ),
          };
        }
      } else {
    ...
    if (customShapeId) {
      ...
      if (customTrayItem.shapeType === "multi-shape") {
        const multiShapeDefaults = isMultiShapeDefaultProps(
        ...
        if (childDefinitions.length > 0) {
          ...
          const childShapes = buildChildShapesFromDefinition(
            childDefinitions,
            baseX,
            baseY,
            {
        ...
        const instanceShape: ShapeCreationInput = {
          ...
          meta: {
            ...cleanMetadata(
              originalShape.meta as Record<string, unknown> | undefined
            ),
            customShapeId,
            instanceId,
            isCustomShapeInstance: true as const,
      ...
      editor.run(
        () => {
          const idsToDelete = new Set<TLShapeId>(
            selectedShapes.map((shape) => shape.id as TLShapeId)
          );
          validSelectedShapes.forEach((shape) =>
            idsToDelete.add(shape.id as TLShapeId)
          );
          ...
          if (
            customTrayItem.shapeType === "multi-shape" &&
            shapeInstances.length > 1
          ) {
            const instanceIds = shapeInstances
              .map((shape) => shape.id)
              .filter(Boolean) as TLShapeId[];
            const newGroupId = createShapeId();
            editor.groupShapes(instanceIds, {
              groupId: newGroupId,
              select: false,
            });
    ...
    console.log(
      `Created custom shape from ${shapeDescription}:`,
      customTrayItem.label
    );
  }, [
    validSelectedShapes,
    selectedShapes,
    ...
    (e: React.PointerEvent) => {
      if (
        ...
        try {
          ...
          const isCustomShape =
            "createdAt" in trayItem && "version" in trayItem;
          ...
          if (trayItem.shapeType === "multi-shape") {
            ...
            if (childDefinitions.length > 0) {
              ...
              const createdShapeIds = childShapes
                .map((shape) => shape.id)
                .filter(Boolean) as TLShapeId[];
      ...
      <div
        ...
        onKeyDown={(e) => {
          ...
          if (
            ...
            selectedCustomShapeId &&
            isCustomShape(selectedCustomShapeId)
          ) {
      ...
      >
        ...
        >
          ...
          >
            <button
              onClick={
                hasValidShapesSelected ? handleCreateCustomShape : undefined
              }
              disabled={false}
              title={
            ...
            >
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: lucideIcons.plus.replace(
                    /stroke="currentColor"/g,
                    'stroke="#007acc"'
                  ),
                }}
      ...
      {dragPreview && dragState.type === "dragging" && (
        ...
        >
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: (
                allTrayItems.find((item) => item.id === dragPreview.itemId)
                  ?.iconSvg || ""
              ).replace(/stroke="currentColor"/g, 'stroke="#007acc"'),
            }}
          />
        </div>
      )}
